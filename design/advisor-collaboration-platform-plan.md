...
| 2026-07-06 | **Q-ROLES resolved (owner)** — role-hierarchy model settled, unblocking FEAT-RBAC. **(a) Source of truth = hybrid:** derive **Firm** from `branch` + **Group/country** from `country-address` (Q3); read **manager/mentor** designations from a `role` value in the Advisory login token, with a **local override table** as the interim seam until the master team confirms the exact token values. **(b) Management model:** each manager tier gets the **same console + view-as** the Firm Manager has, scoped to its branch — a manager sees/controls only people **at or below** them, inside their own branch (Group Manager → all firms in their country; Global Manager → all countries). **(c) Cross-org toggle = ceiling model:** Global sets the outer limit; Group and Firm may only **tighten** (close-only), never open wider than the tier above — refines the §8 "Global sets the ceiling" rule down to the Firm tier and keeps the firm toggle shipped in FEAT-FIRM-CONSOLE as close-only. **(d) Mentor (top)** = platform super-admin in this app (sees all; master-authoring stays in Advisory's Google tooling, out of scope) — consistent with the interim `mentorRole`. **(e) Role assignment (interim):** only the Mentor/super-admin sets manager designations in the override table (no self-promotion). **(f) Client (bottom) = no access** to this app (a Client-role token is not admitted; Clients receive outputs via Advisory). **Still master-team:** confirm the exact `role` claim values (and whether roles ever arrive via a profile API) — the hybrid design builds against a seam regardless. |
| 2026-07-15 | **FEAT-CROSSORG-GROUPS residual resolved (owner, 5 decisions)** — how the §8 cross-org wall applies to **groups**. **(1) Browse-yes / join-no:** a sealed org's advisers still **see** every listed group (visibility is the incentive to ask their firm to open up; specialty groups that span orgs stay discoverable) but **cannot join** one outside their reach. **(2) Reach = the group's OWNER's org** — the same one-check rule the marketplace applies to a listing's owner; not per-member (membership churn would make reachability feel random). **(3) Blocked UI = greyed-out button + plain-English note** (never a live-looking button that silently fails — the outreach-modal lesson); the server refuses regardless. **(4) An out-of-reach group page shows member COUNTS only, names hidden** — group pages must not undo the people-search seal. **(5) A posture flip never ejects existing members** — sealing stops NEW cross-org joins only (the marketplace "keep what you bought" principle). The group **chat/message routes are walled the same way** (they previously had no gate and would have bypassed the join block). Same-org non-member chat access remains SEC-THREAD-ACL (INT-AUTH). |
