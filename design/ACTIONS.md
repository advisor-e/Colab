...
| FEAT-CROSSORG-GROUPS | P2 | Cross-org gating for **groups & spaces** + the **manager toggle UI** to flip a firm open/closed | Feature | Q-ROLES resolved 2026-07-06 — now blocked only on the **FEAT-RBAC build** (toggle = ceiling model) · see Done note |
...
| ID | P | Title | Notes |
|----|---|-------|-------|
| FEAT-RBAC | P2 | Role hierarchy / RBAC enforcement | **Slices 1–2 landed 2026-07-06** (see Done). **Remaining slices, each its own PR:** ③ **cross-org ceiling** enforcement (Global sets the limit; Group/Firm tighten only) — this is FEAT-CROSSORG-GROUPS + the manager toggle UI; ④ **audit-viewer admin gate** (FEAT-AUDIT-UI); ⑤ real **Client-token rejection** (needs INT-AUTH); ⑥ wire the real Advisory `role` claim (master-team) to retire the interim override table + the demo preview personas. |
