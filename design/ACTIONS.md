...
|----|---|-------|-------------------|
| P1-TEST | P1 | Frontend component + e2e tests still to add | **Updated 2026-07-02 — major progress.** Backend + mixins now covered and gated: overall **~99% lines** (176 tests, 16 suites). Routes **100% lines** (people/translate/health/templates); repository **99%**; proxy/db/middleware/utils **100%**; mixins (`localeMixin`, `speechMixin`) **100% lines**. `jest.config.js` now enforces `coverageThreshold` gates — routes ≥90%, mixins ≥80%, security utils 100%, plus a global floor — so coverage cannot regress (enforced in the pre-commit hook **and** CI, since `npm test` collects coverage). **Remaining:** (a) `.vue` component tests — **harness now added 2026-07-02** (`@vue/test-utils@1.3.6` + `@vue/vue2-jest@27.0.0`, Pug + jsdom; first component `PageHelp.vue` at 100%). **All page + component tests added 2026-07-02:** the `PageHelp` component **and every page** — `connections`, `discover`, `marketplace`, `profile`, `messages`, `groups/_id`, `groups/new`, home `index` (mount + `fetch`/`$buefy`/`$router`/`$route`/`$i18n` mocks; Buefy/nuxt tags stubbed via `localVue`). **232 tests, 25 suites.** **Only remaining P1-TEST piece:** (b) Playwright critical-journey e2e (real browser click-through against a running app). |
