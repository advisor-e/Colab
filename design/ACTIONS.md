...
| FEAT-GROUP-SHARED-CONTENT | P2 | Group: creator **attaches shared pages/templates** (dropdown) for members to collaborate/edit; members see the group's **template list** on the group page | Feature | **Display slice DONE 2026-07-03** (see Done — FEAT-SHARED-WORKSPACE); the **attach-via-dropdown** editor UI remains · Part of T1-SPACES |
| FEAT-SHARED-WORKSPACE-ATTACH | P2 | The **"+ attach a page"** picker so a group creator / 1:1 pair can add Advisor-e pages to their Shared workspace (display already built) | Feature | After Q-PAGE-URL (real IDs) · uses the marketplace tool-picker pattern |
