...
- **Cross-org engagement policy** (plan §8): the wall is **enforced** on discovery/connection/
  outreach/profile, with a **three-level ceiling** (brand → country → branch, most-closed-wins;
  a lower level may only tighten) set per manager tier from the console. Default posture is a
  **config flip** (`config/integration.js → CROSS_ORG`, closed/opt-in). *(Still to gate:
  marketplace — `ACTIONS.md` SEC-MARKET-CROSSORG; groups/spaces — FEAT-CROSSORG-GROUPS/T1.)*
